...
contract("RPS", async (accounts) => {
    ...
    it("Playing 2 players", async () => {
        ...
        for (i of [...Array(3).keys()]) {
            ...
            rps.createRound(web3.utils.soliditySha3(player1.choice, SECRET), {from: player1.address, value: betAmount});
            let lastRound = await rps.roundCount();
    ...
    it("Test error when creating round with bet lower than minimum bet", async () => {
        ...
        try {
            await rps.createRound(web3.utils.soliditySha3(player1.choice, SECRET), {from: player1.address, value: minimumBet - 1});
        }
        catch(e) {
            ...
            await rps.createRound(web3.utils.soliditySha3(player1.choice, SECRET), {from: player1.address, value: minimumBet});
            return;
        }
